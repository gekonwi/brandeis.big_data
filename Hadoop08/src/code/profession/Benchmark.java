...
public class Benchmark {
	...
	public static void split() throws IOException { 
		//method used to split profession_train into *train.known, *train.test, and *train.sol
		BufferedReader in = new BufferedReader(new FileReader("profession_train.txt"));
		BufferedWriter outKnown = new BufferedWriter(new FileWriter("prof_train.known.txt"));
		BufferedWriter outTest = new BufferedWriter(new FileWriter("prof_train.test.txt"));
		BufferedWriter outSol = new BufferedWriter(new FileWriter("prof_train.sol.txt"));
		
		
		long counter = 0;
		
		String line = in.readLine();
		while (line != null) {								//loop as long as there is more input 
	...
	public static String truncate(String line, int test) {
		...
		if (test == 0) {
			return lineParts[0];				//string at index 0 is the first half of truncation(the name) 
		} else {						
			return lineParts[1];				//string at index 1 is the seecond half(the professions)
		}
	}
}
