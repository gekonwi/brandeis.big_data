...
public class Benchmark {
	...
	public static void main (String[] args) throws IOException {
		...
		BufferedReader in = new BufferedReader(new FileReader("prof_train.result.txt"));	 //result of classification on prof_train.test.
		BufferedReader in2 = new BufferedReader(new FileReader("prof_train.sol.txt"));  	 //solution to prof_train.test.
		...
		while (line !=null) {
			...
			String[] splited = profList.split("\\s+"); //splits string of professions into each profession.
			if (line2.contains(splited[0]) || line2.contains(splited[1]) || line2.contains(splited[2])) {
				correct++;
			}
			counter++;
			line = in.readLine();
