...
public class ProfessionIndexMapredTest {
	MapDriver<Text, Text, Text, StringDouble> mapDriver;
	ReduceDriver<Text, StringDouble, Text, StringDoubleList> reduceDriver;
	MapReduceDriver<Text, Text, Text, StringDouble, Text, StringDoubleList> mapReduceDriver;
	...
	@Test public void testMapper() throws IOException {
		...
		mapDriver.withOutput(new Text("singer"), new StringDouble("boyz", 1));
		mapDriver.withOutput(new Text("musician"), new StringDouble("boyz", 1)); 
		mapDriver.withOutput(new Text("singer"), new StringDouble("debutlp", 1)); 
		mapDriver.withOutput(new Text("musician"), new StringDouble("debutlp", 1));
		mapDriver.withOutput(new Text("singer"), new StringDouble("parent", 1));
		mapDriver.withOutput(new Text("musician"), new StringDouble("parent", 1)); 
		mapDriver.withOutput(new Text("singer"), new StringDouble("year", 13)); 
		mapDriver.withOutput(new Text("musician"), new StringDouble("year", 13));
		
		// must get object representations instead of mapDriver.runTests() 
		// because of lack of equals/hashCode method 
		List<Pair<Text, StringDouble>> input = mapDriver.run(); 
		List<Pair<Text, StringDouble>> output = mapDriver.getExpectedOutputs();
	...
	public void testReducer() throws IOException {
		...
		List<StringDouble> list1 = new ArrayList<StringDouble>(); 
		list1.add(new StringDouble("boyz", 1)); 
		list1.add(new StringDouble("debutlp", 1));
		list1.add(new StringDouble("parent", 1)); 
		list1.add(new StringDouble("year", 13)); 
		list1.add(new StringDouble("parent", 2));
		list1.add(new StringDouble("year", 20));
	...
	@Test public void testProfessionUtils() {
		String test = "2 Pistols : singer, musician";
		List<String> testList = new ArrayList<String>(); testList.add(test);
	...
	*/
	...
	assertEquals("resultant list of professions should include all of expected"
					+ "professions as key values", true, correct);
	}
